...
컴퓨터라는 큰 집 안에 여러 개의 **특수 목적용 방**을 만든다고 상상해 보세요.
...
2. **활성화:**
  * Windows: `call myenv\Scripts\activate`
  * Mac/Linux: `source myenv/bin/activate`
3. **패키지 설치:** `pip install requests` (이 설치는 'myenv' 안에만 저장됩니다)
4. **종료:** `deactivate`
...
| 특징 | pip | Conda |
| --- | --- | --- |
| **관리 대상** | 파이썬 패키지만 관리 | 파이썬 패키지 + 시스템 레벨 라이브러리 |
...
파이썬의 가상환경 관리 도구인 **venv**와 **Conda**는 목적과 사용 환경에 따라 뚜렷한 차이가 있습니다.
...
# 3. 필수 라이브러리 한 번에 설치
seaborn
mamba install pandas numpy pytorch matplotlib seaborn scikit-learn jupyter -y
