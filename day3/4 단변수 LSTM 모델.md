...
LSTM(Long Short-Term Memory) 모델 내부에는 크게 두 가지 목적의 활성화 함수가 존재합니다. 정보를 전달하는 **'엔진'** 역할의 함수와, 정보를 얼마나 통과시킬지 결정하는 **'수문(Gate)'** 역할의 함수입니다.
...
> tanh는 relu보다 연산량이 조금 더 많고, 층이 깊어질 경우 앞쪽 층으로 갈수록 학습 에너지가 줄어드는 '기울기 소실'이 발생할 가능성이 relu보다 높습니다. 상황에 따라 tanh 또는 relu으로 선택하여 결과를 선택해주세요. 

### 4. `Dense(32, activation='tanh')`

* **특징 추출:** LSTM이 요약한 64개의 정보 중 핵심적인 32개를 다시 추출합니다
