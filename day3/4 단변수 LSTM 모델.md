...
* **중요:** LSTM은 반드시 **(데이터 개수, 타임스텝, 특성 수)** 라는 3차원 입력을 받아야 합니다. 여기서 `(window_size, 1)`은 그중 뒤의 두 자리를 정의한 것입니다.

#### 3. `LSTM(64, activation='relu')`

* **의미:** 64개의 유닛(노드)을 가진 LSTM 레이어입니다.
* **역할:** 시계열 데이터 내부의 **장기/단기 패턴**을 기억하고 학습합니다. 일반적인 신경망과 달리 데이터의 '순서'를 인식합니다.
* **activation='relu'**: 활성화 함수로 `ReLU`와 `tanh` 있습니다. 여기서 전력략으로 값이 0 이상만 존재 하므로 ReLU를 사용합니다. 음수도 존재하는 경우 tanh를 사용합니다.

##### 1. tanh (Hyperbolic Tangent): 시계열의 고전적 선택

LSTM 레이어의 **기본(Default) 활성화 함수**입니다.

* **특징:** 출력값을 **-1에서 1 사이**로 압축합니다. 0을 중심으로 데이터가 분포하므로 학습이 매우 안정적입니다.
* **시계열에서의 역할:** 데이터의 흐름을 제어하는 '게이트(Gate)' 역할에 최적화되어 있습니다. 값을 적절히 걸러주고 보존해야 하는 LSTM의 구조적 특성상, 값이 너무 커지지 않게 꽉 잡아주는 `tanh`가 유리합니다.
* **단점 (기울기 소실):** 입력값이 너무 크거나 작으면 기울기(Gradient)가 0에 가까워져, 아주 먼 과거의 정보를 학습하지 못하는 문제가 발생할 수 있습니다.

##### 2. ReLU (Rectified Linear Unit): 현대적이고 빠른 선택

최근 깊은 신경망에서 가장 선호되는 함수로, 시계열에서도 자주 시도됩니다.

* **특징:** **음수는 0으로, 양수는 그대로($x$)** 내보냅니다.
* **시계열에서의 역할:** 전력 사용량, 주가 등 **항상 양수인 데이터**를 다룰 때 직관적입니다. 또한, 값이 아무리 커져도 기울기가 1로 유지되기 때문에 아주 긴 시퀀스(Long sequence)를 학습할 때 에너지를 끝까지 전달하는 능력이 뛰어납니다.
* **단점 (기울기 폭주):** `tanh`처럼 상한선(1)이 없기 때문에, 순환 구조를 거치면서 값이 기하급수적으로 커져 모델이 망가지는 **Exploding Gradient** 현상이 발생할 수 있습니다.

---

##### 3. 시계열 학습 시 선택 가이드

| 비교 항목 | **tanh** (안정 지향) | **ReLU** (성능/속도 지향) |
| --- | --- | --- |
| **LSTM 기본값** | 예 (Internal State 제어용) | 아니오 (별도 설정 필요) |
| **학습 속도** | 상대적으로 느림 (지수 연산) | 매우 빠름 (단순 비교) |
| **데이터 특성** | 등락이 심하고 음수가 섞인 데이터 | 항상 0 이상인 데이터 (전력 사용량 등) |
| **추천 상황** | 모델이 처음이고 안정성이 중요할 때 | 층을 깊게 쌓고 학습 속도를 높이고 싶을 때 |
